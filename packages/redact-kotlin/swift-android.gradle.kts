...
// in the repo-root mise.toml) builds libRedactAndroid.so per ABI, copies it +
// libc++_shared.so + libLiteRt.so into src/main/jniLibs, and stages the
// model into the optional :redact-tflite-resources module's resources. This task
...
val buildSwiftNatives by tasks.registering(Exec::class) {
    ...
    outputs.dir("$rootDir/redact-tflite-resources/src/main/resources")
}
